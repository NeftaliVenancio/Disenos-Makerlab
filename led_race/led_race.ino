...
void setup() {
  ...
  if ((digitalRead(sel1)==0)) //push switch 1 on reset for activate physics
  {ENABLE_RAMP=1;
   ...
   if ((digitalRead(sel1)==0)) {VIEW_RAMP=1;} // if retain push switch 1 set view ramp
  ...
  if ((digitalRead(sel2)==0)) {delay(1000); tone(PIN_AUDIO,1000);delay(500);noTone(PIN_AUDIO);delay(500);if ((digitalRead(PIN_P2)==1)) SMOTOR=1;} //push switch 2 until a tone beep on reset for activate magic FX  ;-)
